...
# Write the answer in a comment
# Q: Which state has the highest average of cases per county?
# A: Arizona
```
...
```{r} most_covid_death_state <- covid_county %>%
      group_by(state) %>%
      summarise(total_death = sum(death)) %>%
      arrange(desc(total_death))
      
      most_covid_death_state
```
...
```{r}
least_covid_death_state <- covid_county %>%
  group_by(state) %>%
  summarise(total_death = sum(death)) %>%
  arrange(total_death)
...
```{r} 
averages_county_death_by_state <- covid_county %>%
  group_by(state) %>%
  summarise(average_county_death = mean(cases)) %>%
  arrange(desc(average_county_death))

averages_county_death_by_state
