...
# Write the answer in a comment
# Q: What was the average number of cases per county in each state? Which state had the lowest average?
# A:Vermont had the lowest average 
...
number_of_counties<- covid_county %>%
  group_by(state) %>%
  summarise(total_counties =n()) %>%
...
```{r}
#Q: Which state had the lowest number of deaths on average per county? What is the average?
#A: Alaska, 1.307

state_deaths_per_county <- covid_county %>%
  group_by(state) %>%
  summarise(deaths_in_average_couny = mean(deaths)) %>%
  arrange(deaths_in_average_couny)

state_deaths_per_county
